...
m3 = lmer(val ~ freq + (freq | subject), data = df2 %>% mutate(val = val - 4, freq = freq - 4))
summary(m3)

# test whether freq > midpoint (4), controlling for value
m4 = lmer(freq ~ val + (val | subject), data = df2 %>% mutate(val = val - 4, freq = freq - 4))
summary(m4)

## add info to df for better statistical analysis (see supplement)
