...
## Analysis code for Study 7
# Note: This code must be run with R version 3 (it's been specifically tested with R 3.6.3).
# It will break with R version >=4.

# If you downloaded this code from ResearchBox, before running this script you may have to unzip the file "study7_s1.csv.zip",
# and then rename the file that emerges to "study7_s1.csv".
...
# Load data
df.demo.raw = read.csv(paste0('study', expt, '_demo.csv'), stringsAsFactors = F) %>% arrange(subject) %>% mutate(total_time_real = total_time / 60000)
...
for (subj in 1:length(subjlist)) {
  ...
  exclude = df.demo.temp$write_down == 'Yes' || df.s2.subj.temp$comp_check_pass < 1 ||
    ...
    subj.name == 'f06edc9c52a0c4d183da9af9222bba30' || # someone who glitched
...
df.s1.subj.filt %>% group_by(cond) %>%
  summarize(pctCorrect_choice.m = mean(pctCorrect_choice), pctCorrect_choice.se = se(pctCorrect_choice))
...
df.words.filt = df.words.filt %>% mutate(s1_value_rank = max(s1_value_rank, na.rm = T) - s1_value_rank + 1,
                                         ...
                                         cond.fac = factor(cond, c(1,0), c('Gains', 'Losses')),
                                         s1_value_fac = cut(s1_value, 6, 1:6))
...
# graph predictions of hypotheses
...
ggplot(graph.s1.full %>% filter(cond.fac == 'Gains'), aes(x = s1_value, y = in.cs)) +
  ...
  scale_x_continuous(breaks = c(min(graph.s1.full$s1_value), max(graph.s1.full$s1_value))) +
  scale_y_continuous(breaks = c(.3,.45), limits = c(.3,.45)) +
  betterLine(graph.s1.full %>% filter(cond.fac == 'Gains'), in.cs ~ s1_value)
...
ggplot(graph.s1.full %>% filter(cond.fac == 'Losses'), aes(x = s1_value, y = in.cs)) +
  ...
  scale_x_continuous(breaks = c(min(graph.s1.full$s1_value), max(graph.s1.full$s1_value))) +
  scale_y_continuous(breaks = c(.3,.45), limits = c(.3,.45)) +
  betterLine(graph.s1.full %>% filter(cond.fac == 'Losses'), in.cs ~ s1_value)
...
ggplot(graph.s2 %>% filter(cond.fac == 'Gains'), aes(x = s2_value, y = in.cs.m)) +
  ...
  scale_y_continuous(breaks = c(.2, .5), limits = c(.18, .57)) +
  scale_x_continuous(breaks = c(2, 25), limits = c(2,25)) +
  betterLine(graph.s2 %>% filter(cond.fac == 'Gains'), in.cs.m ~ s2_value)
...
ggplot(graph.s2 %>% filter(cond.fac == 'Losses'), aes(x = s2_value, y = in.cs.m)) +
  ...
  scale_y_continuous(breaks = c(.2, .5), limits = c(.18, .57)) +
  scale_x_continuous(breaks = c(2, 25), limits = c(2,25)) +
  betterLine(graph.s2 %>% filter(cond.fac == 'Losses'), in.cs.m ~ s2_value)
