...
## Analysis code for Pilot (reported in supplement)
...
library(groundhog)
pkgs = c('dplyr', 'tidyr', 'ggplot2', 'ggExtra', 'lme4', 'lmerTest', 'mlogit', 'stringdist', 'rje', 'QuantPsyc')
...
df = read.csv('pilot_data.csv') %>% filter(DistributionChannel != 'preview', Status == 0) %>%
